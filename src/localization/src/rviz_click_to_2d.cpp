...
int main(int argc, char **argv) {
  ...
  while (ros::ok()) {
    geometry_msgs::PoseStamped rpyPose;  // Declare here
    geometry_msgs::PoseStamped rpyGoal;  // Declare here

    // Publish rpyPose and rpyGoal only after filling them in callbacks
    pub2.publish(rpyPose);
    pub.publish(rpyGoal);
